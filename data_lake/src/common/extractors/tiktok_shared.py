...
def run_extraction(
    ...
    if not date_clicked:
        print("[ERROR] Could not find date selector. Page might have changed.")
        print("[INFO] Attempting to continue without date selection...")
        # Don't raise error - try to proceed with default date range
    
    # Only try to select 365 days if we clicked the date selector
    if date_clicked:
        ...
        print("[INFO] Skipping date range selection - using default range")
