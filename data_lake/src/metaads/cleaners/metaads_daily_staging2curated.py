...
STAGING_DIR = PROJECT_ROOT / "3_staging" / "metaads"
CURATED_DIR = PROJECT_ROOT / "4_curated"
ARCHIVE_DIR = PROJECT_ROOT / "5_archive" / "metaads"
...
def create_performance_log(df: pd.DataFrame) -> pd.DataFrame:
    """Create timeless performance log preserving ALL historical data (no 28-day limit)"""
    
    if df.empty:
        return pd.DataFrame()
    
    # NO FILTERING - preserve all historical data
    ...
    print(f"[CURATED] Created timeless performance log with {len(performance_log)} daily records")
    print(f"[CURATED] Campaigns tracked: {performance_log['campaign_id'].nunique()}")
    
    # Show date range covered
    min_date = performance_log['date'].min()
    max_date = performance_log['date'].max()
    print(f"[CURATED] Performance log covers: {min_date.date()} to {max_date.date()}")
    
    # Show currently active vs inactive campaigns
    if 'is_active' in df.columns:
        active_campaigns = df[df['date'] == max_date]['is_active'].sum()
        total_campaigns = performance_log['campaign_id'].nunique()
        print(f"[CURATED] Campaign status: {active_campaigns} currently active, {total_campaigns - active_campaigns} inactive")
    
    return performance_log

# %%
def load_existing_performance_log() -> pd.DataFrame:
    """Load existing performance log to append new data to"""
    performance_file = CURATED_DIR / "metaads_campaigns_performance_log.csv"
    
    if performance_file.exists():
        print(f"[CURATED] Loading existing performance log...")
        existing_df = pd.read_csv(performance_file, encoding='utf-8')
        existing_df['date'] = pd.to_datetime(existing_df['date'])
        print(f"[CURATED] Loaded {len(existing_df)} existing records")
        return existing_df
    else:
        print(f"[CURATED] No existing performance log found - will create new")
        return pd.DataFrame()

# %%
def merge_with_existing_log(new_log: pd.DataFrame, existing_log: pd.DataFrame) -> pd.DataFrame:
    """Merge new performance data with existing log, removing duplicates"""
    ...
    combined = combined.sort_values(['date', 'campaign_id'])
    
    print(f"[CURATED] Merged data: {len(existing_log)} existing + {len(new_log)} new = {len(combined)} total records")
    
    return combined

# %%
def archive_existing_files():
    """Archive existing curated files with timestamp (only metadata, not performance log)"""
    
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    
    # Only archive the metadata file - performance log is append-only
    files_to_archive = [
...
def main():
    ...
    # Write performance log (append mode for timeless log)
    if not performance_log.empty:
        # Load existing log and merge
        existing_log = load_existing_performance_log()
        final_performance_log = merge_with_existing_log(performance_log, existing_log)
        
        performance_file = CURATED_DIR / "metaads_campaigns_performance_log.csv"
        final_performance_log.to_csv(performance_file, index=False, encoding='utf-8')
        print(f"[CURATED] ✅ Wrote timeless performance log: {performance_file} ({len(final_performance_log)} total records)")
        
        # Show date range
        min_date = final_performance_log['date'].min().date()
        max_date = final_performance_log['date'].max().date()
