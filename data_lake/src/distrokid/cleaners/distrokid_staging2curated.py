...
if cur_path.exists() and fhash(cur_path) == dfhash(merged):
    print("<- No changes - curated already up-to-date.")
else:
    if cur_path.exists():
    ...
    print(f"[OK] Curated updated -> {cur_path.relative_to(PROJECT_ROOT)}")
...
if bank_src.exists():
    if bank_dst.exists() and fhash(bank_dst) == fhash(bank_src):
        print("<- Bank details unchanged.")
    else:
        if bank_dst.exists():
            ts = datetime.datetime.now().strftime("%Y%m%dT%H%M%S")
            shutil.copy2(bank_dst, ARCHIVE / f"dk_bank_details_{ts}.csv")
        shutil.copy2(bank_src, bank_dst)
        print(f"[OK] Bank details promoted -> {bank_dst.relative_to(PROJECT_ROOT)}")
