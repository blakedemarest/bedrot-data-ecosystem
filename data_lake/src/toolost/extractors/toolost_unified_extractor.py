...
OUTPUT_DIR = PROJECT_ROOT / "1_landing" / "toolost"
...
class TooLostExtractor:
    ...
    async def manual_login(self, page):
        ...
        # Auto-fill credentials if environment variables are set
        toolost_email = os.environ.get("TOOLOST_EMAIL")
        toolost_password = os.environ.get("TOOLOST_PASSWORD")
        
        if toolost_email and toolost_password:
            try:
                print("[TOOLOST] Attempting auto-fill login...")
                
                # Wait for email input field using type selector
                ...
                await email_input.fill(toolost_email)
                await asyncio.sleep(0.2)
                print(f"[INFO] Auto-filled email: {toolost_email}")
                
                # Fill password
                password_input = page.locator("input[type='password']")
                await password_input.clear()
                await asyncio.sleep(0.1)
                await password_input.fill(toolost_password)
                await asyncio.sleep(0.2)
                print("[INFO] Auto-filled password")
    ...
    async def ensure_platform_tab(self, page):
        """Ensure we're on the 'By Platform' tab."""
        try:
            # Check if we're already on the platform tab
            platform_tab = page.locator("a.v-tab--active:has-text('By Platform')")
            if await platform_tab.count() > 0:
                print("[TOOLOST] Already on 'By Platform' tab")
                return
            
            # Click on the By Platform tab
            print("[TOOLOST] Clicking 'By Platform' tab...")
            await page.click("a.v-tab:has-text('By Platform')")
            await asyncio.sleep(1)
            
            # Wait for tab to be active
            await page.wait_for_selector("a.v-tab--active:has-text('By Platform')", timeout=5000)
            print("[TOOLOST] Successfully navigated to 'By Platform' tab")
            
        except Exception as e:
            print(f"[TOOLOST] Error navigating to platform tab: {e}")
    
    async def ensure_spotify_selected(self, page):
        """Ensure Spotify platform is selected."""
        try:
            # Check if Spotify is already active
            spotify_active = page.locator("div.label.active:has-text('Spotify')")
            if await spotify_active.count() > 0:
                print("[TOOLOST] Spotify already selected")
                return
            
            # Click on Spotify if not selected
            print("[TOOLOST] Selecting Spotify platform...")
            spotify_label = page.locator("div.label:has-text('Spotify')").first
            if await spotify_label.count() > 0:
                await spotify_label.click()
                await asyncio.sleep(1)
                print("[TOOLOST] Spotify platform selected")
            else:
                print("[TOOLOST] WARNING: Could not find Spotify platform selector")
                
        except Exception as e:
            print(f"[TOOLOST] Error selecting Spotify platform: {e}")
    
    async def select_date_range(self, page):
        """Select 'This Year' date range."""
        try:
            print("[TOOLOST] Looking for date range selector...")
            
            # Look for various possible date picker selectors
            date_selectors = [
                "input.v-input",  # Vuetify input
                "div.v-input__slot",  # Vuetify input slot
                "div.v-select",  # Vuetify select
                "[role='button'][aria-haspopup='listbox']",  # ARIA dropdown
                "button:has-text('Date')",
                "button:has-text('Period')",
                "*:has-text('This year'):visible"  # If already showing
            ]
            ...
            for selector in date_selectors:
                try:
                    ...
                    if await element.is_visible():
                        await element.click()
                        clicked = True
                        print(f"[TOOLOST] Clicked date selector: {selector}")
                        break
                except:
                    continue
            ...
            if clicked:
                ...
                # Try to click "This year" in the dropdown
                try:
                    # Look for the exact text in v-list-item__title
                    await page.click("div.v-list-item__title:has-text('This year')", timeout=3000)
                    print("[TOOLOST] Selected 'This year' date range")
                except:
                    # Try alternative selectors
                    try:
                        await page.click("text='This year'", timeout=2000)
                        print("[TOOLOST] Selected 'This year' date range (alt)")
                    except:
                        print("[TOOLOST] Could not find 'This year' option")
            else:
                print("[TOOLOST] WARNING: Could not find date range selector")
    ...
    async def switch_to_apple(self, page):
        ...
        try:
            print("[TOOLOST] Switching to Apple platform...")
            
            # Check if Apple is already active
            apple_active = page.locator("div.label.active:has-text('Apple')")
            if await apple_active.count() > 0:
                print("[TOOLOST] Apple already selected")
                return
            
            # Click on Apple label
            apple_label = page.locator("div.label:has-text('Apple')").first
            if await apple_label.count() > 0:
                await apple_label.click()
                await asyncio.sleep(1)
                print("[TOOLOST] Switched to Apple Music")
            else:
                # Try alternative selectors
                alt_selectors = [
                    "div:has-text('Apple'):not(.active)",
                    "*:has-text('Apple Music')",
                ...
                for selector in alt_selectors:
                    try:
                        ...
                        if await element.is_visible():
                            await element.click()
                            print(f"[TOOLOST] Switched to Apple Music using: {selector}")
                            break
                    except:
                        continue
                else:
                    print("[TOOLOST] WARNING: Could not find Apple platform selector")
    ...
    async def extract(self):
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                # Ensure we're on the "By Platform" tab
                await self.ensure_platform_tab(page)
                
                # Set up response capture
                self.setup_response_capture(page)
                
                # Ensure Spotify is selected
                await self.ensure_spotify_selected(page)
