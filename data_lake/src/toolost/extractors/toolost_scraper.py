...
OUTPUT_DIR = Path(PROJECT_ROOT) / "2_raw" / "toolost"
...
async def _wait_for_login(page):
    print("[TOOLOST] Checking authentication status...")
    ...
    try:
        ...
        print("[TOOLOST] Already authenticated!")
        return
    except Exception:
        pass
    ...
    for idx, text in enumerate(texts):
        if re.search(r"Your \\(\d{2}-\d{4}\\) Sales report has been generated", text):
            ...
            if await buttons.count() > idx:
                ...
                dest = Path(PROJECT_ROOT) / "1_landing" / "toolost" / "streams"
