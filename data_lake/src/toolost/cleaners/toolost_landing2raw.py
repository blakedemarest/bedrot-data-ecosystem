...
for file in sorted(landing_dir.glob("*.json")):
    ...
    if not valid:
        print(f"[ERROR] {file.name:40} {msg}")
    ...
    if tgt.exists():
        ...
        if file_hash == existing_hash:
            ...
            print(f"<- {file.name:40} already in raw (hash match)")
            ...
            if time_diff.days > 1:
                print(f"   [WARN] Landing file is {time_diff.days} days newer but content is identical")
            ...
            print(f"[UPDATE] {file.name:40} content changed, versioning as {tgt.name}")
    
    # Copy new or changed file
    shutil.copy2(file, tgt)
    print(f"[OK] {file.name:40} -> {tgt.name}")
